...
async function getContractor() {
  ...
  if (authError || !authData.user) {
    return { supabase, user: null, contractor: null, response: fail("Authentication required.", 401) };
  }
  ...
  if (profileError) return { supabase, user: null, contractor: null, response: fail(profileError.message, 500) };
  if (!profile?.active || profile.role !== "contractor") {
    return { supabase, user: null, contractor: null, response: fail("Contractor access required.", 403) };
  }
  ...
  if (contractorError) return { supabase, user: null, contractor: null, response: fail(contractorError.message, 500) };
  if (!contractor || contractor.email?.toLowerCase() !== profile.email?.toLowerCase()) {
    return { supabase, user: null, contractor: null, response: fail("Active contractor record not found.", 403) };
  }

  return { supabase, user: profile, contractor, response: null };
}
...
export async function GET() {
  ...
  const { data: assignments, error: assignmentsError } = await supabase
    ...
    .select("id, job_id, contractor_id, status, assigned_at, accepted_at, completed_at, removed_at, notes")
  ...
  if (jobIds.length === 0) {
    return NextResponse.json({ assignments: [], jobs: [], lineItems: [], photos: [], documents: [], signoffs: [] });
  }
  ...
  const [jobsResult, lineItemsResult, photosResult, documentsResult, signoffsResult] = await Promise.all([
    ...
    supabase
      .from("job_signoffs")
      .select("id, job_id, representative_name, representative_title, signed_at, approved, notes, created_at")
      .in("job_id", jobIds)
      .order("created_at", { ascending: false }),
  ]);
...
export async function POST(request: Request) {
  ...
  if (action === "assignment_status") {
    ...
    const { error: rpcError } = await supabase.rpc("contractor_update_assignment_status", {
  ...
  if (action === "line_item") {
    ...
    const { data, error } = await supabase
      .from("job_line_items")
      .insert({ job_id: jobId, description, quantity, unit_price: unitPrice, total_price: totalPrice, notes: String(body.notes ?? "") || null })
      .select("id, job_id, description, quantity, unit_price, total_price, notes")
      .single();
  ...
  if (action === "photo") {
    ...
    const { data, error } = await supabase
      .from("job_photos")
      .insert({ job_id: jobId, uploaded_by: contractor.id, photo_type: String(body.photo_type ?? "work_performed"), storage_path: storagePath, caption: String(body.caption ?? "") || null })
      .select("id, job_id, photo_type, storage_path, caption, created_at")
      .single();
  ...
  if (action === "document") {
    ...
    const { data, error } = await supabase
      .from("job_documents")
      .insert({ job_id: jobId, uploaded_by: contractor.id, document_type: String(body.document_type ?? "work_performed"), storage_path: storagePath, file_name: String(body.file_name ?? storagePath.split("/").pop() ?? "document") })
      .select("id, job_id, document_type, storage_path, file_name, created_at")
      .single();
