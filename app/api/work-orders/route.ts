...
export async function POST(request: Request) {
  ...
  catch { return fail("Request body must be valid JSON."); }
  ...
  const { data: property, error: propertyError } = await supabase
    .from("properties")
    .select("id, apartment_company_id, name, address, city, state, zip, service_area")
    .eq("id", propertyId)
    .eq("apartment_company_id", requestedCompanyId)
    .maybeSingle();
  ...
  const quantity = Number(body.pricing_quantity ?? 1);
  ...
  const { data: workOrder, error: insertError } = await supabase
    ...
    .select("id, work_order_number, status, pricing_type, unit_price, pricing_quantity, pricing_total, is_estimate")
    .single();
