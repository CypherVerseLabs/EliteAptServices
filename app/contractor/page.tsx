...
export default function ContractorPage() {
  ...
  async function action(payload: Record<string, unknown>) {
    ...
    try {
      const response = await fetch("/api/contractor/jobs", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      const json = await response.json();
  ...
  async function upload(jobId: string, kind: "photo" | "document", file: File | undefined) {
    ...
    try {
      const form = new FormData(); form.set("job_id", jobId); form.set("kind", kind); form.set("file", file);
      const response = await fetch("/api/jobs/files", { method: "POST", body: form });
      const json = await response.json();
      if (!response.ok) throw new Error(json.error ?? "Upload failed.");
      setMessage(`${kind === "photo" ? "Photo" : "Document"} uploaded successfully.`); await load();
  ...
  return (
    <main className="min-h-screen bg-slate-50 p-6 lg:p-8">
      <div className="mx-auto max-w-5xl">
        ...
        <p className="mt-2 text-sm text-slate-500">Review assigned work, accept assignments, record Work Performed, upload job evidence, and complete Jobs.</p>
        ...
        {data.jobs.length === 0 ? <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-10 text-center text-slate-500">No Jobs are currently assigned to you.</div> : <div className="mt-8 space-y-6">
          {data.jobs.map((job) => {
            const assignment = assignmentFor(job.id); const items = itemsFor(job.id); const signoff = signoffFor(job.id); const key = (actionName: string) => `${actionName}:${job.id}`;
            return <section key={job.id} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                {assignment?.status === "assigned" && <><button disabled={busy === key("assignment_status")} onClick={() => void action({ action: "assignment_status", job_id: job.id, status: "accepted" })} className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Accept Job</button><button disabled={busy === key("assignment_status")} onClick={() => void action({ action: "assignment_status", job_id: job.id, status: "declined" })} className="rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 disabled:opacity-50">Decline</button></>}
              ...
              <div className="mt-6 border-t border-slate-200 pt-6">
                ...
                <div className="mt-3 space-y-2">{items.length === 0 && <p className="text-sm text-slate-500">No work items recorded yet.</p>}{items.map((item) => <div key={item.id} className="rounded-xl bg-slate-50 p-3 text-sm"><div className="flex justify-between gap-4"><span className="font-semibold">{item.description}</span><span>${Number(item.total_price ?? 0).toFixed(2)}</span></div><p className="mt-1 text-slate-500">{item.quantity} × ${Number(item.unit_price ?? 0).toFixed(2)}</p></div>)}</div>
              ...
              <div className="mt-6 border-t border-slate-200 pt-6">
                <h3 className="font-bold text-slate-900">Job Evidence</h3><p className="mt-1 text-xs text-slate-500">Images: JPG, PNG, or WebP. Documents: PDF, JPG, or PNG. Maximum 10 MB.</p>
